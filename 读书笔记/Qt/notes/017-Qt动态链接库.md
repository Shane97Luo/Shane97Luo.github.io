...
QT对各种平台的动态链接库编程技术都有包装，QT把这种技术统一命名为共享库(Shared Libraries)。**通过使用Qt包装过的类和宏，可以编写跨平台的共享库和插件**--当然，这只是源代码级别的跨平台，你不要指望用MSVC编译出来的DLL，能集成到ARM平台的Linux程序上面——这是一个很美很美的理想哦。
